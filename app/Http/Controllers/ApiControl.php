    ...
    public function detailbiayaproduksi($id)
    {
      ...
      if ($a->count() > 0) {
        ...
        foreach ($list_komposisi as $key => $value) {
          ...
          $list_bahan["data"][] = [$value->master_bb->id_bb,$value->master_bb->nama,$value->harga_bahan,$value->rasio,($value->jumlah*$row->jumlah),$jml." ".$value->master_bb->master_satuan->nama_satuan,number_format(($value->rasio*$value->jumlah)),number_format(($harga_satuan*$jml))];
        }
